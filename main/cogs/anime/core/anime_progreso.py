...
def formatear_progreso(usuarios, anime_key, server_id=None):
    if not usuarios:
    ...
    for uid, _ in ordenados:
        ...
        # 🔥 CONSULTA REAL AL DROPEADOS_SERVER.JSON (server-scoped cuando se provee)
        dropeado = usuario_dropeo_anime(uid, anime_key, server_id=str(server_id) if server_id else None)
